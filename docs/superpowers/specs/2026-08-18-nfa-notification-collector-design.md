...
The off-main design has an unavoidable non-zero Android process-death window between callback return and the first Room transaction; the app therefore makes no zero-loss claim for that interval. All failures after the worker begins are contained and attempt bounded QUARANTINED persistence without blocking the main callback.
